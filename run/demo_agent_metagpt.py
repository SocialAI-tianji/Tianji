...
def initialize_sidebar(scenes,sharedData):
    with st.sidebar:
        ...
        st.checkbox('启用网络搜索',value=st.session_state["enable_se"], key="check", on_change=flip)
    ...
    if user_input := st.chat_input():
        ...
        if intent_ans == "None":
            ...
            if not has_empty_values(sharedData.scene_attribute):
                ...
                if st.session_state["enable_se"] == True:
                    ...
                    final_ans_sa = (await role_answerBot.run(str(sharedData.message_list_for_agent))).content
