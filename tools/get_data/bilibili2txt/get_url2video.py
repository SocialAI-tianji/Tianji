...
def download_file(url: str, output_dir: Optional[str], output_filename: Optional[str], lux_path: str, playlist: bool, title: Optional[str] = None, convert_audio: bool = False, timeout: int = 3600) -> None:
    """
    ...
    def execute_command():
        ...
        if process.returncode == 0 and "100.00%" in output_text:
            ...
            if convert_audio and output_filename:
                convert_to_audio(os.path.join(output_dir, output_filename+'.mp4'), url)
        else:
...
def convert_to_audio(video_file: str, url: str) -> None:
    ...
    if process.returncode == 0:
        ...
        try:
            ...
            log_error(os.path.dirname(video_file), url, os.path.basename(video_file), str(e))
    else:
        print(f"Conversion failed: {process.stderr}")
        log_error(os.path.dirname(video_file), url, os.path.basename(video_file), process.stderr)
...
def download_from_csv(csv_file: str, output_dir: str, lux_path: str, playlist: bool, convert_audio: bool, timeout: int) -> None:
    """
    ...
    with open(csv_file, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            url = row['URL']
            title = row['Title']
            filename = title.replace(' ', '_').replace('/', '_').replace('\\', '_')  # Assuming video files, change extension if necessary
            if os.path.exists(os.path.join(output_dir, filename+'.mp3')) or os.path.exists(os.path.join(output_dir, filename+'.mp4')):
                print(f"已存在该文件{filename}，跳过！")
                continue
            time.sleep(2)
            download_file(url, output_dir, filename, lux_path, playlist, title, convert_audio, timeout)
...
def main() -> None:
    ...
    parser.add_argument('--convert_audio', '-c', action='store_true', help='Convert the downloaded video to audio')
    parser.add_argument('--timeout', '-t', type=int, help='Timeout in seconds for the download process', default=6000)

    args = parser.parse_args()

    if args.url:
        download_file(args.url, args.output_dir, args.output_filename, args.lux_path, args.playlist, convert_audio=args.convert_audio, timeout=args.timeout)
    elif args.csv:
        download_from_csv(args.csv, args.output_dir, args.lux_path, args.playlist, convert_audio=args.convert_audio, timeout=args.timeout)
    else:
