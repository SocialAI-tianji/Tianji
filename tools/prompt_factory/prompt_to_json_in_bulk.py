...
"""
folder_path = (
    r"C:\Users\yhd\PycharmProjects\TianjiOrignal\test\prompt\aigame\zhipu"  # 替换成您的文件夹路径
)
    ...
    for foldername, subfolders, filenames in os.walk(folder_path):
        for filename in filenames:
            if filename.endswith(".md"):
                ...
                json_output = md_file_to_json_with_examples(
                    filepath, id=id, heading=heading
                )
                input_dir, input_file = os.path.split(promptpath)
                input_file_base, _ = os.path.splitext(input_file)

                output_path = (
                    r"C:\Users\yhd\PycharmProjects\TianjiOrignal\tianji\prompt"
                )
                ...
                json_file_output = os.path.join(
                    json_file_output_path, input_file_base + ".json"
                )
                with open(json_file_output, "w", encoding="utf-8") as file:
                    json.dump(
                        json.loads(json_output), file, ensure_ascii=False, indent=4
                    )
                print(json_output)
